resource "aws_iam_role_policy" "bedrock_access_policy_ragcore" {
  role   = aws_iam_role.ragcore_service_account.id
  name   = "${local.instancename}-bedrock-access-policy-ragcore"
  policy = <<EOF
    {
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_role_policy" "bedrock_access_policy_backend" {
  role   = aws_iam_role.backend_service_account.id
  name   = "${local.instancename}-bedrock-access-policy-backend"
  policy = <<EOF
    {
    "Version": "2012-10-17",
    "Statement": [
