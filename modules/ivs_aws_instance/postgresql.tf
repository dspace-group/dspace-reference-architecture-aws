...
resource "aws_db_instance" "ivs_authentication" {
  ...
  instance_class                      = var.db_instance_type_ivs
  identifier                          = local.db_ivs_id
  db_name                             = "ivsauthentication"
  ...
  monitoring_role_arn                 = aws_iam_role.rds_enhanced_monitoring_role_ivs[0].arn # [RDS.9] Database logging should be enabled
  ...
  tags                                = var.tags
  depends_on = [
    aws_cloudwatch_log_group.db_ivs_authentication
  ]
  timeouts {
    create = "2h"
    delete = "2h"
...
resource "aws_iam_role_policy_attachment" "rds_enhanced_monitoring_policy_ivs" {
  count      = var.enableIVSAuthentication ? 1 : 0
  role       = aws_iam_role.rds_enhanced_monitoring_role_ivs[0].name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonRDSEnhancedMonitoringRole"
}

resource "aws_cloudwatch_log_group" "db_ivs_authentication" {
  count             = var.enableIVSAuthentication ? 1 : 0
  name              = "/aws/rds/instance/${local.db_ivs_id}/postgresql" # CAUTION: the name is predetermined by AWS RDS. Do not change it. Otherwise AWS will create a new log group without retention and encryption.
